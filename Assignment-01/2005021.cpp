...
class compare {
    public:
        bool operator() (pair<int, search_node*> a, pair<int, search_node*> b){
            if(a.first == b.first){
                return a.second->move_count > b.second->move_count;
            }
...
void A_star_search(int k, vector<vector<char>> &grid, vector<vector<char>> &goal, int blank_row, int blank_col, int (*heuristic_function) (vector<vector<char>> &board)){
    priority_queue<pair<int, search_node*>, vector<pair<int, search_node*>>, compare> open_list;
